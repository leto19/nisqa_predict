        ...
        if self.apply_bias_loss:
            ...
            if self.do_update:
                ...
                for db_name in self.db.unique():
                    print('--> bias updated for {}'.format(db_name))
                    db_idx = (self.db==db_name).to_numpy().nonzero()
                    print("db_idx",db_idx)
                    y_hat_db = y_hat[db_idx]
                    print("y_hat_db",y_hat_db.shape)
                    y_db = y[db_idx]
                    print("y_db",y_db.shape)
                    if not np.isnan(y_db).any():
                        ...
                        if not db_name==self.anchor_db:
                            ...
                            #print("self.b",self.b[db_idx,:len(b_db)])
